...
public:
    ...
    int instart,int inend,map<int,int>& mp){
        ...
        root->left=build(preorder,prestart+1,prestart+nums_left,inorder,instart,inroot-1,mp);
        root->right=build(preorder,prestart+nums_left+1,preend,inorder,inroot+1,inend,mp);
        return root;
    }
